...
//===========================================================================
	...
	//===========================================================================
			...
			<div style="text-align:center;">
				<div class="woodPanelH" style="font-weight:bold;text-align:center;clear:both;font-variant:small-caps;padding:2px 0px;">Special</div>
				${legacyStr}
...
//===========================================================================
		{name:'stats',nameD:'Stats',icon:3,build:function(el){
			...
			if (l('statsData'))
			{
				l('statsData').innerHTML=`<div class="woodPanelH" style="font-weight:bold;text-align:center;clear:both;font-variant:small-caps;padding:2px 0px;">Stats</div>${viewLogButton}
					...
					<line></line>
					<div class="woodPanelH" style="font-weight:bold;"><p>Upgrades owned: <b>${B(G.upgradesN)}/${B(G.upgradesTotal)}</b> (${B((G.upgradesN/G.upgradesTotal)*100,1)}%)</p></div>
				`;})+`<p>${viewUpgradesButton}</p><line></line>`+
				G.selfUpdatingText(e=>{
					var milk=G.milks[Math.min(G.milks.length-1,Math.floor(G.achievsN/25))];
					return `
					<div class="woodPanelH" style="font-weight:bold;"><p>Achievements unlocked: <b>${B(G.achievsN)}/${B(G.achievsTotal)}</b> (${B((G.achievsN/G.achievsTotal)*100,1)}%)</p></div>
...
//===========================================================================
		{name:'misc',nameD:'Misc',icon:4,build:function(el){
			...
			let reloadApp=()=>{
				...
				if (typeof admob!=='undefined') admob=null;//this prevents "Alert - invalid action" messages - note: no longer working with previous setup as of 2022; now using the diagnostics plugin to restart instead which does not cause this issue
				/*if (navigator && navigator.app && navigator.app.loadUrl) navigator.app.loadUrl("file:///android_asset/www/index.html");
				else window.location=initialHref;*/
				if (cordova && cordova.plugins && cordova.plugins.diagnostic) cordova.plugins.diagnostic.restart(G.Crash,false);
				else window.location=initialHref;
			};
			...
			if (ALPHA_FEATURES) str+=`<div style="font-size:9px;text-shadow:1px 1px 0px #000;">The alpha version features ${ALPHA_FEATURES}.</div>`;
			else str+=`<div style="font-size:9px;text-shadow:1px 1px 0px #000;">The alpha version currently has no extra features and is only used for compatibility.</div>`;
			...
			el.innerHTML='<div class="section shadeUpInner shadeDownInner"><div class="woodPanelH" style="font-weight:bold;text-align:center;clear:both;font-variant:small-caps;padding:2px 0px;">Misc</div>'+str+'</div>';
			/*
			var str='';
			var support='';
